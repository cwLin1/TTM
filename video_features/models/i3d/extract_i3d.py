...
class ExtractI3D(BaseExtractor):
    ...
    def __init__(self, args, data_dir) -> None:
        ...
        self.resize_transforms = torchvision.transforms.Compose([
            ...
            torchvision.transforms.Resize(self.min_side_size),
            torchvision.transforms.CenterCrop(self.min_side_size),
            PILToTensor(),
            ToFloat(),
        ])
        self.i3d_transforms = {
    ...
    @torch.no_grad()
    def extract(self, seg_id) -> Dict[str, np.ndarray]:
        ...
        while cap.isOpened() and frame_num < end_frame:
            ...
            if frame_num-start_frame < len(bbox):
        ...
        # Padding
        while len(rgb_stack) - 1 < self.stack_size:
            rgb_stack.append(torch.zeros(1, 3, 256, 256))
